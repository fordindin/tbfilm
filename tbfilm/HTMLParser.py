...
def meta(page):
    d = etree.parse(page)
    metadata = d.xpath('//*[@id="metadata"]')[0]
    return metadata

def epcount(page):
    page = page.replace(" & ", " &amp; ")
    d = etree.fromstring(page)
    epcount = d.xpath('//*/span[@id="epcount"]')[0].text
    return epcount.strip(", ")

def unseen(page):
    page = page.replace(" & ", " &amp; ")
    d = etree.fromstring(page)
    series = d.xpath('//*/div[@class="myseriesbox"]')
    rlist = []
    for s in series:
        l = etree.ElementTree(s)
        name = l.xpath('//*[@class="myseriesent"]')[0].text
        total = len(l.xpath('//*[@class="myseriesblock"]|//*[@class="myseriesblockc"]'))
        shortname = re.match("/Series/(.*)",
                l.xpath('//*[@class="myseriesico"]/a/@href')[0]).groups()[0]
        rlist.append( (name, total, shortname) )
    rlist.sort(key=itemgetter(1), reverse=True)
    return rlist
